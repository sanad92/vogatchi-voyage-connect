...
export default function BookingsTable() {
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('bookings')
        .select('id, booking_number, customer_name, destination, start_date, status, selling_price, currency')
  ...
  return <Card><CardHeader><CardTitle>آخر الحجوزات</CardTitle></CardHeader><CardContent>
    {query.isLoading ? <p>جاري تحميل الحجوزات…</p> : query.error ? <p role="alert">تعذر تحميل الحجوزات. حاول مرة أخرى.</p> :
      <Table><TableHeader><TableRow><TableHead>المرجع</TableHead><TableHead>العميل</TableHead><TableHead>الوجهة</TableHead><TableHead>تاريخ الخدمة</TableHead><TableHead>الحالة</TableHead><TableHead>قيمة البيع</TableHead><TableHead>التفاصيل</TableHead></TableRow></TableHeader>
        <TableBody>{query.data?.map(booking => <TableRow key={booking.id}>
          <TableCell>{booking.booking_number}</TableCell><TableCell>{booking.customer_name}</TableCell><TableCell>{booking.destination || '—'}</TableCell>
