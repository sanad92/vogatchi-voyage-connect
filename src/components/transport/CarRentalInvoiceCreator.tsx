...
import { CarRental } from "@/types/transport";
import { calculateFinancialBreakdown } from "@/utils/calculationHelpers";
import { useCurrencyHelper } from "@/hooks/useCurrencyHelper";

interface CarRentalInvoiceCreatorProps {
  booking: CarRental;
  open: boolean;
  onClose: () => void;
}

const CarRentalInvoiceCreator = ({ booking, open, onClose }: CarRentalInvoiceCreatorProps) => {
  const [formData, setFormData] = useState({
    subtotal: booking.total_rental_cost || 0,
    vat_rate: 14,
    discount_amount: 0,
  ...
  const createInvoiceMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success('تم إصدار فاتورة إيجار السيارة');
      onClose();
    },
    onError: (error) => {
      console.error('Error creating car rental invoice:', error);
      toast.error('خطأ في إصدار فاتورة إيجار السيارة');
    }
  });
...
export default CarRentalInvoiceCreator;
