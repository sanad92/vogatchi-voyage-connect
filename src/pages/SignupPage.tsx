...
const SignupPage = () => {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (result.error) {
      const errorMsg = (result.error as { message?: string; code?: string })?.message
        || (result.error as { message?: string; code?: string })?.code
        || '';
      if (errorMsg.includes('already') || errorMsg.includes('user_already_exists')) {
        setError('هذا البريد الإلكتروني مسجل بالفعل. يرجى تسجيل الدخول بدلًا من ذلك.');
      } else {
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            {loggedInWithoutOrganization
              ? 'الحساب الحالي تم إنشاؤه بالفعل ويحتاج فقط إلى ربط مؤسسة.'
              : 'سجّل شركتك وابدأ تجربتك المجانية لـ 14 يوم'}
          </p>
        </div>
