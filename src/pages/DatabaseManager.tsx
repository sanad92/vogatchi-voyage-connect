        ...
        <Alert variant="destructive" className="max-w-md">
          ...
          <div>
            ...
            <div className="text-foreground mb-1">
              هذه الصفحة متاحة فقط لمديري المنصة.
            </div>
  ...
  return (
    <div className="w-full px-4 md:px-6 lg:px-8 py-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <Badge variant="destructive">مدير منصة فقط</Badge>
        <Button
          variant="outline"
          size="sm"
          className="ml-auto"
      ...
      <p className="mb-5 text-muted-foreground max-w-2xl">
        ...
        <span className="text-foreground font-bold">
          {' '}
          الآن يمكنك تصفح كل جدول أو تصدير بياناته أو فتح محرر SQL.
        </span>
      </p>
          ...
          <table className="min-w-[450px] w-full bg-card border rounded-xl shadow-sm divide-y">
            ...
            <tbody>
              {availableTables.map((table) => (
                <tr key={table.name} className="border-b hover:bg-muted transition">
                  <td className="p-3 font-mono text-foreground">{table.name}</td>
                  <td className="p-3 text-center">{table.rowCount ?? '?'}</td>
                  <td className="p-3 text-muted-foreground">{table.description}</td>
                  <td className="flex gap-2 items-center justify-center p-3">
                    <Button size="sm" onClick={() => setSelectedTable(table.name)}>
                      تصفح
                    </Button>
                  </td>
                </tr>
