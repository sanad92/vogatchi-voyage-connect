...
function renderAdminSidebar(string $currentPage = 'dashboard', string $dir = 'rtl'): void
{
    ...
    echo '<aside id="appSidebar" class="fixed inset-y-0 z-40 w-72 -translate-x-full border-e border-slate-200/80 bg-white/95 shadow-sm transition-transform duration-200 dark:border-slate-800 dark:bg-slate-900/95 lg:translate-x-0 rtl:translate-x-full lg:rtl:translate-x-0">';
    ...
    foreach ($items as $item) {
        ...
        $baseClass = 'flex items-center rounded-xl px-3.5 py-2.5 text-sm font-medium transition-colors';
        ...
        echo '<i class="' . htmlspecialchars($item['icon'], ENT_QUOTES, 'UTF-8') . ' w-5 text-[0.95rem]"></i>';
