-- Booking-level profitability and settlement cockpit.
...
BEGIN
  ...
  SELECT
    coalesce(sum(CASE WHEN a.account_code LIKE '4%' THEN l.credit - l.debit ELSE 0 END), 0),
    coalesce(sum(CASE WHEN a.account_code LIKE '5%' THEN l.debit - l.credit ELSE 0 END), 0),
    coalesce(sum(CASE WHEN a.account_code LIKE '6%' THEN l.debit - l.credit ELSE 0 END), 0),
...
COMMENT ON FUNCTION public.get_booking_profit_cockpit(uuid) IS
  'Authorized booking P&L cockpit with allocation-safe settlement totals, direct costs, commissions, ledger comparison and drilldowns.';
