...
CREATE OR REPLACE FUNCTION public.sop_set_department(
  _user_id uuid,
  _department public.sop_department DEFAULT NULL,
...
BEGIN
  ...
  -- current active workload in the sales pipeline (used for fairness)
  ...
  ELSIF _old.department IS DISTINCT FROM _department THEN
    -- transfer: move record, reset fairness cursor, reconcile load
    UPDATE public.sop_department_members
     ...
     WHERE id = _old.id;
  ELSE
    -- same department: only availability/load reconcile, keep fairness cursor
    UPDATE public.sop_department_members
...
REVOKE ALL ON FUNCTION public.sop_set_department(uuid, public.sop_department, boolean, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.sop_set_department(uuid, public.sop_department, boolean, text) TO authenticated;

CREATE OR REPLACE FUNCTION public.sop_set_availability(
  _user_id uuid,
...
REVOKE ALL ON FUNCTION public.sop_set_availability(uuid, boolean, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.sop_set_availability(uuid, boolean, text) TO authenticated;
