...
BEGIN
  ...
  -- Canonical stage mapping: asking Reservations for a price moves the lead to
  -- `pricing_requested` while the Sales owner keeps ownership of the file.
  UPDATE public.sop_leads
     SET owner_department = 'reservations'::public.sop_department,
         stage = CASE WHEN stage IN ('new','qualified','assigned')
                      THEN 'pricing_requested'::public.sop_lead_stage ELSE stage END,
         updated_at = now()
   WHERE id = _lead;
...
CREATE OR REPLACE FUNCTION public.sop_publish_pricing(_request uuid, _valid_until date DEFAULT NULL, _recommendation text DEFAULT NULL)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $fn$
...
BEGIN
  ...
  -- V1: rate validity is optional and never blocks publishing.
  vu := COALESCE(_valid_until, r.price_valid_until, (current_date + 30));
