-- Route-aware cash flow reports. Cash and bank may use custom company accounts.
...
BEGIN
  ...
  IF _cash_account_id IS NOT NULL AND NOT EXISTS (
    ...
    WHERE a.id = _cash_account_id
      ...
      AND a.id IN (public.get_org_account_route(_org_id, 'cash'), public.get_org_account_route(_org_id, 'bank'))
  ) THEN
...
REVOKE ALL ON FUNCTION public.get_cash_flow_v2(uuid,date,date,text,uuid,uuid,text) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.get_cash_flow_details_v2(uuid,date,date,text,uuid,uuid,text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_cash_flow_v2(uuid,date,date,text,uuid,uuid,text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_cash_flow_details_v2(uuid,date,date,text,uuid,uuid,text) TO authenticated, service_role;
