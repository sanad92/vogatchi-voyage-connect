-- Bank account opening baseline (source-of-truth copy of the live DDL/RPC/RLS).
-- Idempotent. No backfill, no change to current_balance, no data corrections.

ALTER TABLE public.bank_accounts
  ADD COLUMN IF NOT EXISTS opening_balance numeric,
  ADD COLUMN IF NOT EXISTS opening_balance_date date,
  ADD COLUMN IF NOT EXISTS opening_balance_note text,
  ADD COLUMN IF NOT EXISTS opening_balance_set_by uuid,
...
GRANT ALL ON public.bank_account_baseline_audit TO service_role;
...
CREATE POLICY bank_account_baseline_audit_read
  ON public.bank_account_baseline_audit
  FOR SELECT TO authenticated
  USING (public._can_read_org_finance(organization_id));
...
REVOKE ALL ON FUNCTION public._can_manage_bank_baseline(uuid) FROM PUBLIC, anon;
