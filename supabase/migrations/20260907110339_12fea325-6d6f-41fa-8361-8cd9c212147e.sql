ALTER TABLE public.hotel_bookings ADD COLUMN IF NOT EXISTS booking_id uuid REFERENCES public.bookings(id) ON DELETE SET NULL;
ALTER TABLE public.flight_bookings ADD COLUMN IF NOT EXISTS booking_id uuid REFERENCES public.bookings(id) ON DELETE SET NULL;
ALTER TABLE public.transport_bookings ADD COLUMN IF NOT EXISTS booking_id uuid REFERENCES public.bookings(id) ON DELETE SET NULL;
ALTER TABLE public.car_rentals ADD COLUMN IF NOT EXISTS booking_id uuid REFERENCES public.bookings(id) ON DELETE SET NULL;
...
CREATE INDEX IF NOT EXISTS idx_car_rentals_booking_id ON public.car_rentals(booking_id);
...
BEGIN
  ...
  SELECT COALESCE(SUM(selling_amount), 0), COALESCE(SUM(cost_amount), 0)
    INTO v_sell, v_cost
  FROM (
    -- legacy lightweight itinerary rows
    ...
    -- real service bookings linked to this booking file
    UNION ALL
  ...
  FOR r IN
    ...
    UNION ALL
    SELECT 'طيران: ' || COALESCE(airline_name, '') || ' - ' || COALESCE(booking_reference, ''),
           COALESCE(total_cost, 0)
    FROM public.flight_bookings WHERE booking_id = p_booking_id AND COALESCE(total_cost, 0) > 0
    UNION ALL
    SELECT 'نقل: ' || COALESCE(vehicle_type, '') || ' - ' || COALESCE(booking_reference, ''),
           COALESCE(total_cost, 0)
