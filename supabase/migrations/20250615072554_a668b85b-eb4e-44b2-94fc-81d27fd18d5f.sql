...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'unique_employee_profile'
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tgname = 'sync_profile_to_employee'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger
    WHERE tgname = 'sync_employee_to_profile'
  ) THEN
...
DROP FUNCTION IF EXISTS link_user_to_employee(uuid, uuid);
...
DROP FUNCTION IF EXISTS unlink_user_from_employee(uuid);
