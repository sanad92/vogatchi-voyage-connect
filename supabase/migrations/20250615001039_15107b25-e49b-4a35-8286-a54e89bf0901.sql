...
ALTER TABLE public.payment_orders ADD COLUMN IF NOT EXISTS bank_account_id UUID REFERENCES public.bank_accounts(id);
ALTER TABLE public.payment_orders ADD COLUMN IF NOT EXISTS exchange_rate NUMERIC(15,6) DEFAULT 1.00;
ALTER TABLE public.payment_orders ADD COLUMN IF NOT EXISTS amount_in_account_currency NUMERIC(15,2);

-- إضافة عمود سعر الصرف للفواتير
ALTER TABLE public.invoices ADD COLUMN IF NOT EXISTS exchange_rate_to_egp NUMERIC(15,6) DEFAULT 1.00;
ALTER TABLE public.invoices ADD COLUMN IF NOT EXISTS total_amount_egp NUMERIC(15,2);
ALTER TABLE public.invoices ADD COLUMN IF NOT EXISTS exchange_rate_to_usd NUMERIC(15,6);
ALTER TABLE public.invoices ADD COLUMN IF NOT EXISTS total_amount_usd NUMERIC(15,2);
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_trigger
    WHERE tgname = 'update_bank_balance_trigger'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_trigger
    WHERE tgname = 'invoice_multi_currency_trigger'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_trigger
    WHERE tgname = 'payment_order_currency_trigger'
  ) THEN
