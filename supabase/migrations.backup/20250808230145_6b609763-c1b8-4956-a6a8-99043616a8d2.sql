...
VALUES (
  ...
  ARRAY['عن فوجاتشي', 'شركة سياحة', 'تاريخ الشركة', 'رؤية', 'مهمة', 'سفر'],
  true
) ON CONFLICT (slug) DO UPDATE SET
  name = EXCLUDED.name,
  description = EXCLUDED.description,
...
VALUES (
  ...
  ARRAY['حجز رحلة', 'حجز فندق', 'حجز طيران', 'رحلات سياحية', 'فوجاتشي'],
  true
) ON CONFLICT (slug) DO UPDATE SET
  name = EXCLUDED.name,
  description = EXCLUDED.description,
